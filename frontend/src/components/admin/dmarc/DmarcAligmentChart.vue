    ...
    <div v-if="alignments">
        <v-card>
            <v-toolbar dense elevation="0" class="mr-2">
                ...
                >
                <v-btn icon="mdi-arrow-right" size="x-small" @click="nextWeek">
                </v-btn>
            </v-toolbar>
            <v-card-text>
                <v-row>
        ...
        <v-expansion-panels v-model="panel" class="mt-2">
            <v-expansion-panel :title="$gettext('Detail')">
                <v-expansion-panel-text>
                    ...
                    <DmarcSourceTable
                        :title="
                            $gettext('Partially trusted sources / Forwarders')
                        "
                        color="orange"
                        :total="stats.trusted"
    ...
    <div v-else>
        ...
        >
            <div>
                {{
                    $gettext(
                        'DMARC support does not seem to be enabled for this domain.'
                    )
                }}
            </div>
            <div>
