...
If you need to rebuild you virtualenv, you can check this [part of the documentation](/rebuild_venv)
...
The documents generated by the `pdfcredentials` plugin are now stored
using authenticated encryption (with tamper detection) and a dedicated
encryption key.

Documents created by previous versions remain readable, but you should
convert them to the new format by running the following command once,
after the upgrade:

``` shell
$ python manage.py reencrypt_pdf_credentials
```

The command only converts old-format files and can safely be run
several times.

### Optional: dedicated encryption key

By default, encryption keys are derived from Django's `SECRET_KEY`. You
can now decouple the encryption of stored data (credential documents,
IMAP migration passwords) from `SECRET_KEY` by defining a dedicated
high-entropy key in `settings.py`:

``` python
MODOBOA_ENCRYPTION_KEY = 'a long random string (32+ characters)'
```

::: warning
Only define this setting on a fresh installation or **before** any
credential document or IMAP migration is created: data encrypted with
...
SPECTACULAR_SETTINGS = {
    ...
    'TITLE': 'Modoboa API',
    'VERSION': None,
    'SERVE_PERMISSIONS': ['rest_framework.permissions.IsAuthenticated'], # [!code --]
    'SERVE_PERMISSIONS': ['rest_framework.permissions.AllowAny'], # [!code ++]
...
* Add the following lines to `RQ_QUEUES` variable:
...
RQ_QUEUES = {
    ...
    'dovecot': {  # [!code ++]
        'HOST': REDIS_HOST,  # [!code ++]
        'PORT': REDIS_PORT,  # [!code ++]
        'DB': REDIS_QUOTA_DB,  # [!code ++]
        'URL': REDIS_URL,  # [!code ++]
    },  # [!code ++]
    'privileged': {  # [!code ++]
        'HOST': REDIS_HOST,  # [!code ++]
...
### New `cron_config.py` file

* Create the file `<modoboa_instance_dir>/instance/cron_config.py` with the following content:
...
Replace `<minute>`, `<hour_start>` and `<hour_end>` variables with random values.
...
::: tip INFORMATION
Legacy web interface has been removed and almost all extensions have been integrated to the core Modoboa package.
:::
...
* Run the following command:
  ...
  $ python manage.py load_initial_data
  ````
* Update your `webserver <webserver>` configuration
...
This will silence a [warning sent by django-ckeditor](https://github.com/modoboa/modoboa/issues/3316).
...
If you were using [modoboa-postfix-autoreply](https://github.com/modoboa/modoboa-postfix-autoreply)
extension, you need to perfome a few commands:

* If you are using Postegresql (default option with the installer), run as a root user:
...
The location of this file is `/srv/modoboa/instance/instance` by default.
...
* The `modoboa-postfix-autoreply` and `modoboa-sievefilters` plugins have been merged into the core.

  Add `'modoboa.postfix_autoreply'` and `modoboa.sievefilters` to `MODOBOA_APPS`:
  ...
  MODOBOA_APPS = (
     ...
     'modoboa_postfix_autoreply', # [!code --]
     'modoboa_sievefilters', # [!code --]
     'modoboa.postfix_autoreply', # [!code ++]
     'modoboa.sievefilters', # [!code ++]
  )
  ```

  ::: warning
  Remove any reference to `'modoboa_postfix_autoreply'` AND `modoboa_sievefilters` in this same variable.
  :::

* Due to the changes to the authentication flow, you need to replace MIDDLEWARE with the following:
...
* Replace `'modoboa.core.drf_authentication.JWTAuthenticationWith2FA'` by `'oauth2_provider.contrib.rest_framework.OAuth2Authentication'`
  at the top of the list of `DEFAULT_AUTHENTICATION_CLASSES` (inside `REST_FRAMEWORK` section):

  ``` python
  'DEFAULT_AUTHENTICATION_CLASSES': (
     'modoboa.core.drf_authentication.JWTAuthenticationWith2FA',  # [!code --]
     'oauth2_provider.contrib.rest_framework.OAuth2Authentication', # [!code ++]
     'rest_framework.authentication.TokenAuthentication',
     'rest_framework.authentication.SessionAuthentication',
  ),
  ```

Save your modifications at this point and run the `post upgrade commands <post_upgrade_commands>` now.

Then, apply this last modification to your settings:

* Run the commands indicated `in the first section <post_upgrade_commands>` then add the following content after `MEDIA_ROOT`:
...
[url] with [path] (in `/srv/modoboa/instance/instance/urls.py` by default):
...
supervisord for the installation (this apply if you used `modoboa-installer`):
...
MODOBOA_APPS = (
   ...
   'modoboa.maillog',
   'modoboa_dmarc', # [!code --]
   'modoboa_pdfcredentials', # [!code --]
   'modoboa_imap_migration', # [!code --]
   'modoboa.dmarc', # [!code ++]
   'modoboa.pdfcredentials', # [!code ++]
   'modoboa.imap_migration', # [!code ++]
)
```

And remove any reference to `'modoboa_dmarc'`, `'modoboa_pdfcredentials'` or `'modoboa_imap_migration'` in this samevariable.
...
AUTHENTICATION_BACKENDS = (
   ...
   'modoboa.imap_migration.auth_backends.IMAPBackend', # [!code ++]
)
```
    ...
    TEMPLATES = [
        {
            ...
            'OPTIONS': {
                'context_processors': [
                    ...
                    'modoboa.core.context_processors.new_admin_url', # [!code ++]
                ],
                'debug': False,
            },
        },
...
* You now have the possibility to customize the url of the new-admin interface:
  To do so please head up to `the custom configuration chapter <customization>` (advanced user).
...
* You can edit the `DEFAULT_THROTTLE_RATES` to whatever value suits you:
    ...
    * `ddos_lesser` is for per api endpoint and per user or per ip if not logged.
      This is for api endpoint that arelighter.
    * `login` the number of time an ip can attempt to log.
...
INSTALLED_APPS = (
    ...
    'rest_framework',
    'rest_framework.authtoken',
    'drf_spectacular', # [!code ++]
    'phonenumber_field', # [!code ++]
    'django_otp',
    'django_otp.plugins.otp_totp',
    'django_otp.plugins.otp_static',
)
...
INSTALLED_APPS = (
    ...
    'rest_framework.authtoken',
    'django_otp', # [!code ++]
    'django_otp.plugins.otp_totp', # [!code ++]
    'django_otp.plugins.otp_static', # [!code ++]
)
```
...
MIDDLEWARE = (
    'x_forwarded_for.middleware.XForwardedForMiddleware',
    'django.contrib.sessions.middleware.SessionMiddleware',
    'django.middleware.locale.LocaleMiddleware', # [!code ++]
    'django.middleware.common.CommonMiddleware',
    'django.middleware.csrf.CsrfViewMiddleware',
    'django.contrib.auth.middleware.AuthenticationMiddleware',
...
A new `policy daemon <policy_daemon>` has been added.
...
Don't forget to configure `_policyd_config <postfix>` if you want to use this feature.
...
If you want to manage inactive accounts, look at `inactive_accounts` .
...
BASE_DIR = os.path.realpath(os.path.dirname(os.path.dirname(__file__))) # [!code ++]
```

## Version 1.8.0

Modoboa now relies on [Django's builtin password validation system](https://docs.djangoproject.com/en/1.10/topics/auth/passwords/#module-django.contrib.auth.password_validation)
...
PASSWORD_MIN_LENGTH = 8 # [!code --]

PASSWORD_COMPLEXITY = { # [!code --]
    "UPPER": 1, # [!code --]
    "LOWER": 1, # [!code --]
    "DIGITS": 1 # [!code --]
} # [!code --]
...
AUTH_PASSWORD_VALIDATORS = [ # [!code ++]
    ...
    { # [!code ++]
        'NAME': 'django.contrib.auth.password_validation.MinimumLengthValidator', # [!code ++]
    }, # [!code ++]
    { # [!code ++]
        'NAME': 'django.contrib.auth.password_validation.CommonPasswordValidator', # [!code ++]
    }, # [!code ++]
    { # [!code ++]
        'NAME': 'django.contrib.auth.password_validation.NumericPasswordValidator', # [!code ++]
    }, # [!code ++]
    { # [!code ++]
        'NAME': 'modoboa.core.password_validation.ComplexityValidator', # [!code ++]
        'OPTIONS': { # [!code ++]
            'upper': 1, # [!code ++]
            'lower': 1, # [!code ++]
            'digits': 1, # [!code ++]
...
INSTALLED_APPS = (
    ...
    'reversion',
    'ckeditor', # [!code ++]
    'ckeditor_uploader', # [!code ++]
    'rest_framework', # [!code ++]
    'rest_framework.authtoken', # [!code ++]
)
```
...
Edit the `settings.py`  and apply the
following changes:
...
::: warning
You have to upgrade extensions due to [core.User]  model
attribute change ([user.group]  to [user.role] ).
...
To enable it, add `'rest_framework_swagger'` to the `INSTALLED_APPS` variable in `settings.py`  as follows:
...
```python
SWAGGER_SETTINGS = { # [!code ++]
    "is_authenticated": False, # [!code ++]
    "api_version": "1.0", # [!code ++]
    "exclude_namespaces": [], # [!code ++]
    "info": { # [!code ++]
        "contact": "contact@modoboa.com", # [!code ++]
        "description": ("Modoboa API, requires a valid token."), # [!code ++]
        "title": "Modoboa API", # [!code ++]
    } # [!code ++]
} # [!code ++]
```
...
To enhance security, Modoboa now checks the [strength of user passwords](https://github.com/dstufft/django-passwords) .
...
Modoboa now uses the *atomic requests* mode to preserve database consistency
