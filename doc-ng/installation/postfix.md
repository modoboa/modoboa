...
Refer to the [official documentation](http://www.postfix.org/VIRTUAL_README.html)
for more explanation.

## Map files

You first need to create configuration files (or map files) that will be
used by Postfix to lookup into Modoboa tables.
...
`<directory>` is the directory where the files will be stored. Answer
the few questions and you\'re done.

## Configuration

Use the following configuration in the `/etc/postfix/main.cf` file. 

For TLS configuration, you can look at the 
[used configuration](https://github.com/modoboa/modoboa-installer/blob/master/modoboa_installer/scripts/files/postfix/main.cf.tpl)

for [modoboa installer](https://github.com/modoboa/modoboa-installer). (this is just one possible configuration):
...
Replace `<driver>` by the name of the database you use, and `Restart Postfix`.
