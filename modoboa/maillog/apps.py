    ...
    ("general", {
        ...
        "params": collections.OrderedDict([
            ("logfile", {
                ...
                "help_text": _("Path to log file used to collect statistics")
            }),
            ("rrd_rootdir", {
                "label": _("Directory to store RRD files"),
                "help_text": _("Path to directory where RRD files are stored")
            }),
            ("greylist", {
                "label": _("Show greylisted messages"),
                "help_text": _("Differentiate between hard and soft rejects (greylisting)")
            })
        ])
    })
])
...
def load_maillog_settings():
    ...
    param_tools.registry.add(
        "global", forms.ParametersForm, gettext("Statistics"))
    param_tools.registry.add2(
        "global", "maillog", gettext("Statistics"), GLOBAL_PARAMETERS_STRUCT,
