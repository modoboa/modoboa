...
class ComplexityValidator:
    ...
    def validate(self, password, user=None):
        ...
        condition = (
            self.digits > 0
            and sum(1 for char in password if char.isdigit()) < self.digits
        )
        if condition:
        ...
        condition = (
            self.lower > 0
            and sum(1 for char in password if char.islower()) < self.lower
        )
        if condition:
            raise ValidationError(
                ngettext(
        ...
        condition = (
            self.upper > 0
            and sum(1 for char in password if char.isupper()) < self.upper
        )
        if condition:
            raise ValidationError(
                ngettext(
        ...
        condition = (
            self.specials > 0
            and sum(1 for char in password if char in special_characters)
            < self.specials
        )
        if condition:
            raise ValidationError(
    ...
    def get_help_text(self):
        return _(
            "Your password must contain a combination of different " "character types."
        )
