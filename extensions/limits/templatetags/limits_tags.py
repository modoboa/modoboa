...
from modoboa.extensions import limits
...
def resellers_menu(user):
    entries = [{"name" : "new",
                ...
                "url" : reverse(limits.views.new_reseller)},
               {"name" : "remove",
                "label" : _("Remove"),
                "img" : static_url("pics/remove.png"),
                "url" : reverse(limits.views.delete_resellers)
                }]
...
def reseller_actions(user, resid):
    actions = [
        {"name" : "editreseller",
         "url" : reverse(limits.views.edit_reseller, args=[resid]),
         "img" : static_url("pics/edit.png"),
         "title" : _("Edit reseller"),
         "class" : "boxed",
         "rel" : "{handler:'iframe',size:{x:350,y:300}}"},
        {"name" : "editlimits",
         "url" : reverse(limits.views.edit_limits_pool, args=[resid]),
